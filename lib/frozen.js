...
(function() {
  var Collection, Frozen, Model, Options, Validation, extend,
    __slice = [].slice;
  ...
  Validation = (function() {
    ...
    Validation.association = function(prop, value, opts, model) {
  ...
  Options = (function() {
    ...
    Options.prototype.isEqual = function(other) {
      if (this === other) {
        return true;
      }
      return this.validation === other.validation;
    };
  ...
  Model = (function() {
    function Model(attributes, options) {
      var association, error, opts, prop, type, validation, validator, value, _ref, _ref1;
      this.attributes = attributes;
      this.options = options;
      this.attributes = extend({}, this.attributes);
      this.options = extend(new Options(), this.options);
      this.errors = {};
      _ref = this.associations;
      for (prop in _ref) {
        association = _ref[prop];
        value = this.attributes[prop];
        if (value == null) {
          continue;
        }
        if (value instanceof association) {
          if (this.options.isEqual(value.options)) {
            continue;
          }
          value = value.attributes;
        }
        this.attributes[prop] = new association(value, this.options);
      }
      _ref1 = this.validations;
      for (prop in _ref1) {
        validation = _ref1[prop];
        value = this.attributes[prop];
        if (!(this.options.validation === 'force' || (value != null))) {
          continue;
        }
        for (type in validation) {
          ...
          error = validator(prop, value, opts, this);
          if (error == null) {
            continue;
          }
